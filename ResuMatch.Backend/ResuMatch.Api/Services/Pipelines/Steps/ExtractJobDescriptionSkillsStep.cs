...
public class ExtractJobDescriptionSkillsStep : IPipelineStep<PipelineContext, PipelineResult>
{
    ...
    public async Task<PipelineResult> ProcessAsync(PipelineContext context)
    {
        ...
        {
            context.AnalysisResult.ExtractedJobDescriptionSkills = context.JobDescriptionSkills;
        }
        else
        {
            _logger.LogWarning("AnalysisResult is null. Cannot assign ExtractedJobDescriptionSkills.");
        }
        _logger.LogInformation("Extracted {SkillCount} job description skills.", context.JobDescriptionSkills.Count);
